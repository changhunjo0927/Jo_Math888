...
In this course project, I have focused on the causal excursion effect proposed by the paper “Estimating Time-Varying Causal Excursion Effect in Mobile Health with Binary Outcomes, Qian et al. (2021),” which is different from the majority of the literature on causal inference in the longitudinal setting. The causal excursion effect is a contrast of two "excursions" from the current treatment protocol into the future while the majority of causal effect is a contrast of the expected outcome under two fixed treatment trajectories. The causal excursion effect is a suitable estimand for the primary and secondary analyses in the micro-randomized trial because (1) the causal excursion effect approximates an effect in real world implementation of an mHealth intervention, and (2) the causal excursion effect provides an indication of effective "deviations" from the current treatment protocol, or how it might be improved. 

For future research directions, we can first consider extension to treatment with multiple levels since we assumed binary treatment in the paper. Secondly, we focused on estimating the marginal excursion effect, however, introducing random effects to the excursion effect and allowing person-specific predictions would be interesting.
